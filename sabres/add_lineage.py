...
strict_cols = ["Filename", "Lineage", "REF", "POS", "ALT", "REFPOSALT"]
...
def add_pango(file, database, vcall, pango_data):
    ...
    if res_df.empty is True:
        return res_df
    res_df["Filename"] = os.path.splitext(os.path.basename(file))[0]
    str_rm = "|".join(["_t01", "_q20t01", ".varscan.snps"])
    res_df["Filename"] = res_df["Filename"].str.replace(str_rm, "")
    res_df["Lineage"] = (
        res_df["Filename"]
        .map(
            pango_df.drop_duplicates(subset=["name"], keep="first").set_index("name")[
                "Lineage"
            ]
        )
        .fillna("-")
