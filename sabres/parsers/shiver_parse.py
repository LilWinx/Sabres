...
choices = ("A", "C", "G", "T")
neworder = ["Filename", "REF", "POS", "ALT", "REFPOSALT", "Total DP", "Freq"]
...
def shiver_setup(file):
    ...
    csv_df.columns = [
        ...
        "Gap",
        "N",
    ]  # completely replace the column names
    csv_df["Total DP"] = csv_df["A"] + csv_df["C"] + csv_df["G"] + csv_df["T"]
    csv_df[["A%", "C%", "G%", "T%"]] = (
        csv_df[["A", "C", "G", "T"]] / csv_df["Total DP"].values[:, None] * 100
    )
    ...
    if snp_df["Sub"].str.contains("Sub").any():
        ...
        snp_df["ALT"] = np.select(
            sub_conditions, choices, default="This mutation is missing data"
        )
