...
def splitting_vcf(input_file):
    ...
    for column in vcf_df.columns[9:]:
        dynam_split = (
            vcf_df[column].str.split(":", expand=True).add_prefix(column + "_")
        )
        appended_data.append(dynam_split)
    appended_data = pd.concat(appended_data, axis=1)
    combined_df = pd.concat([vcf_df.iloc[:, :9], appended_data], axis=1)
    combined_df = combined_df[
        combined_df.columns.drop(list(combined_df.filter(regex="_1")))
    ]
    combined_df.columns = combined_df.columns.str.rstrip("_0")
    exploded_df = (
        combined_df.assign(alt=combined_df.ALT.str.split(","))
        .explode("alt")
        .reset_index(drop=True)
    )
    exploded_df["ALT"] = exploded_df["alt"]
    exploded_df.drop(
        exploded_df.columns[len(exploded_df.columns) - 1], axis=1, inplace=True
    )
    return exploded_df
